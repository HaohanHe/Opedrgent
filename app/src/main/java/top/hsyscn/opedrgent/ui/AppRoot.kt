...
private fun ToolsDialog(
    ...
    onNotes: () -> Unit,
    onEvolve: () -> Unit,
    onSuggestAutomation: () -> Unit,
    onSuggestCalendar: () -> Unit,
    ...
    var url by rememberSaveable { mutableStateOf("https://duckduckgo.com") }
    AlertDialog(
        onDismissRequest = onDismiss,
        ...
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.fillMaxWidth()) {
                    Button(onClick = { onOpenBrowser(url.trim()) }) { Icon(Icons.Default.Language, null); Spacer(Modifier.width(6.dp)); Text("浏览器") }
                    Button(onClick = onWebSearch, enabled = hasApiKey) { Icon(Icons.Default.Language, null); Spacer(Modifier.width(6.dp)); Text("联网") }
                }
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.fillMaxWidth()) {
                    Button(onClick = onPickPdf) { Icon(Icons.Default.Description, null); Spacer(Modifier.width(6.dp)); Text("PDF") }
                    Button(onClick = onSkills) { Icon(Icons.Default.PlayArrow, null); Spacer(Modifier.width(6.dp)); Text("技能") }
                }
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.fillMaxWidth()) {
                    Button(onClick = onSummary, enabled = hasApiKey, modifier = Modifier.weight(1f)) { Text("摘要") }
                    Button(onClick = onReport, enabled = hasApiKey, modifier = Modifier.weight(1f)) { Text("报告") }
                }
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.fillMaxWidth()) {
                    Button(onClick = onNotes, enabled = hasApiKey, modifier = Modifier.weight(1f)) { Text("整理") }
                    Button(onClick = onEvolve, enabled = hasApiKey, modifier = Modifier.weight(1f)) { Text("进化") }
                }
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.fillMaxWidth()) {
                    Button(onClick = onSuggestAutomation, enabled = hasApiKey, modifier = Modifier.weight(1f)) { Text("自动化建议") }
                    Button(onClick = onSuggestCalendar, enabled = hasApiKey, modifier = Modifier.weight(1f)) { Text("日程建议") }
                }
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.fillMaxWidth()) {
                    Button(onClick = onExportSession, modifier = Modifier.weight(1f)) { Text("导出会话") }
                    Button(onClick = onExportChat, modifier = Modifier.weight(1f)) { Text("导出聊天") }
                }
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.fillMaxWidth()) {
                    Button(onClick = onExportContext, modifier = Modifier.weight(1f)) { Text("导出上下文") }
                    Button(onClick = onExportMemory, modifier = Modifier.weight(1f)) { Text("导出记忆") }
                }
            }
        },
    )
