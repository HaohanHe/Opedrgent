            ...
            try {
                ...
                val prompt = withContext(Dispatchers.IO) {
                    ...
                    buildString {
                        ...
                        appendLine()
                        appendLine("以下字段已从可靠来源（卫星数据库 / 本台设置 / 录音时间）自动预填充，**禁止猜测或随意修改**：")
                        ...
                        appendLine()
                        appendLine("以下字段已预填默认值，若转写文本中有明确信息请覆盖，找不到则保留预填值：")
                        if (filled.date.isNotBlank()) appendLine("  日期: ${filled.date}（文本中有明确日期时覆盖）")
                        if (filled.timeOn.isNotBlank()) appendLine("  开始时间: ${filled.timeOn}（文本中有精确 UTC 时间时覆盖）")
                        appendLine()
